...
class WordlistGenerator:
    ...
    # -----------------------------------------------------------------
    # === Persistencia en archivo (.txt o .gz) dentro de Documentos ===
    # -----------------------------------------------------------------
...
class AdvancedAnalyzer:
    ...
    def verificar_leaks(self, contrasena):
        """Si existe Documentos/leaks_hashes.txt, consulta el SHA1 de la
    ...
    def mostrar_estadisticas_avanzadas(self, contrasena):
        ...
        if leaks is None:
            print(f"{self.colores['morado']}│ {self.colores['gris']}(No se encontró leaks_hashes.txt en Documentos, verificación omitida){self.colores['reset']}")
        elif leaks > 0:
    ...
    def analizar_avanzado_interactivo(self):
        ...
        if modo == "1":
            ...
            ruta = input(f"{self.colores['morado']}│ {self.colores['azul']}Ruta del archivo (relativa a Documentos o absoluta): {self.colores['reset']}").strip()
            if not ruta:
...
{generador.colores['reset']}""")
    ...
    while True:
        ...
        print(f"{generador.colores['morado']}│ {generador.colores['naranja']}[7]{generador.colores['blanco']}  📊 Estadísticas Avanzadas")
