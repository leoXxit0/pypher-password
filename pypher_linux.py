...
class WordlistGenerator:
    ...
    def generar_combinaciones(self, lista1, lista2, separador=""):
        ...
        producto supera MAX_VARIANTES, trunca y avisa. Además, cada lista
        de entrada se limita a MAX_VARIANTES elementos para evitar
        construir en memoria listas de palabras desproporcionadamente
        grandes antes de aplicar el corte del producto cartesiano."""
        if len(lista1) > MAX_VARIANTES:
            print(
                f"{self.colores['naranja']}│ ⚠ La primera lista tiene {len(lista1)} elementos, "
                f"se usarán solo los primeros {MAX_VARIANTES}.{self.colores['reset']}"
            )
            lista1 = lista1[:MAX_VARIANTES]
        if len(lista2) > MAX_VARIANTES:
            print(
                f"{self.colores['naranja']}│ ⚠ La segunda lista tiene {len(lista2)} elementos, "
                f"se usarán solo los primeros {MAX_VARIANTES}.{self.colores['reset']}"
            )
            lista2 = lista2[:MAX_VARIANTES]
