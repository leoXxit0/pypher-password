...
class GeneradorContrasenasCLI:
    def __init__(self):
        ...
        # Obtener (o crear) la carpeta "pypher" donde se guardan los archivos
        self.pypher_path = self._obtener_ruta_pypher()

    def _obtener_ruta_pypher(self):
        """Obtiene la ruta de la carpeta 'pypher' usada para guardar los
        archivos generados por el programa.

        En lugar de depender de carpetas como 'Documentos'/'Documents'
        (cuyo nombre varía según el idioma del sistema y puede no existir
        en todas las distribuciones de Linux), se usa una carpeta fija
        llamada 'pypher' creada en el home del usuario (~/pypher). Esto
        evita problemas de compatibilidad entre distintos sistemas.
        """
        ...
        if not os.path.exists(ruta_pypher):
            try:
                ...
                # Si falla la creación (permisos, etc.), usar el directorio actual
                print(f"{self.colores['rojo']}│ ✗ No se pudo crear ~/pypher, se usará el directorio actual{self.colores['reset']}")
                return "."
        return ruta_pypher
...
"""
        ...
        if subcarpeta:
            ruta_carpeta = os.path.join(self.pypher_path, subcarpeta)
            # Crear subcarpeta si no existe
            if not os.path.exists(ruta_carpeta):
                os.makedirs(ruta_carpeta)
        else:
            ruta_carpeta = self.pypher_path
...
"""
    ...
    def mostrar_historial(self, archivo="historial.txt", subcarpeta=None):
        ...
        if subcarpeta:
            ruta_archivo = os.path.join(self.pypher_path, subcarpeta, archivo)
        else:
            ruta_archivo = os.path.join(self.pypher_path, archivo)
    ...
    def mostrar_acerca_de(self):
        ...
        print(f"\n{self.colores['morado']}╔{'═' * 60}╗{self.colores['reset']}")
        print(f"{self.colores['morado']}│{self.colores['reset']} {self.colores['negrita']}{self.colores['blanco']}ℹ️  ACERCA DE PYPHER{self.colores['reset']}")
        print(f"{self.colores['morado']}╚{'═' * 60}╝{self.colores['reset']}")
        print(f"\n{self.colores['morado']}│{self.colores['reset']} {self.colores['cyan']}🐍 Pypher{self.colores['reset']} - Generador y Analizador de Contraseñas")
        print(f"{self.colores['morado']}│{self.colores['reset']} {self.colores['azul']}Autor:{self.colores['blanco']} leoXxit0")
        print(f"{self.colores['morado']}│{self.colores['reset']} {self.colores['azul']}GitHub:{self.colores['blanco']} https://github.com/leoXxit0")
        print(f"{self.colores['morado']}│{self.colores['reset']} {self.colores['gris']}¡Visita el repositorio para más proyectos, reportar errores{self.colores['reset']}")
        print(f"{self.colores['morado']}│{self.colores['reset']} {self.colores['gris']}o contribuir con mejoras!{self.colores['reset']}\n")
...
║  {generador.colores['azul']}📁 Archivos guardados en: {generador.pypher_path}{generador.colores['cyan']:<22} ║
╚═══════════════════════════════════════════════════════════════════════╝
{generador.colores['reset']}""")
    ...
    while True:
        ...
        print(f"{generador.colores['morado']}│ {generador.colores['morado']}[5]{generador.colores['blanco']}  ℹ️  Acerca del autor")
        print(f"{generador.colores['morado']}│ {generador.colores['rojo']}[6]{generador.colores['blanco']}  🚪 Salir")
        print(f"{generador.colores['morado']}│{generador.colores['reset']}")
