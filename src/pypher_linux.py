...
class GeneradorContrasenasCLI:
    ...
    # =================================================================
    def guardar_archivo(self, contrasena, nombre_archivo, fecha=None, subcarpeta=None, usuario=None):
        ...
        linea_usuario = f"║  Usuario sugerido  : {usuario:<43} ║\n" if usuario else ""

        contenido = f"""╔═══════════════════════════════════════════════════════════════════════╗
║                     🐍 PYPHER - REGISTRO DE CONTRASEÑA                      ║
║                      ───────────────────────────                             ║
║  Fecha de creación : {fecha:<43} ║
{linea_usuario}║  Contraseña        : {contrasena:<43} ║
...
class GeneradorUsuarios:
    """Generador de nombres de usuario sugeridos, pensado para formularios
    web y de banca en línea que exigen reglas comunes de validación:
    longitud fija, mezcla de mayúsculas/minúsculas/números, sin empezar
    ni terminar en número, sin caracteres especiales (salvo '_') y sin
    repeticiones ni secuencias obvias.

    No altera ni depende de ``GeneradorContrasenasCLI``: reutiliza
    únicamente la misma paleta de colores para mantener la estética
    Cyberpunk del resto del programa.
    """

    LONGITUD_USUARIO = 10

    _LETRAS_MAYUS = string.ascii_uppercase
    _LETRAS_MINUS = string.ascii_lowercase
    _DIGITOS = string.digits
    _CARACTERES_PERMITIDOS = _LETRAS_MAYUS + _LETRAS_MINUS + _DIGITOS + "_"

    def __init__(self, colores: dict) -> None:
        """Inicializa el generador reutilizando la paleta de colores
        Cyberpunk existente (no crea una nueva)."""
    ...
    # -----------------------------------------------------------------
    @staticmethod
    def _tiene_repeticion_consecutiva(texto: str) -> bool:
        """Detecta 3 (o más) caracteres idénticos seguidos, p. ej.
        'aaa' o '111'."""
    ...
    @staticmethod
    def _tiene_secuencia(texto: str) -> bool:
        """Detecta secuencias ascendentes o descendentes de 3+ caracteres
        consecutivos del mismo tipo, p. ej. 'abc', 'cba', '123', '321'.
        Los guiones bajos nunca forman parte de una secuencia y las
        letras solo se comparan con letras (y los dígitos con dígitos)."""
        plano = texto.lower()
        for i in range(len(plano) - 2):
            a, b, c = plano[i], plano[i + 1], plano[i + 2]
            if "_" in (a, b, c):
                continue
            mismo_tipo = (a.isalpha() and b.isalpha() and c.isalpha()) or \
                         (a.isdigit() and b.isdigit() and c.isdigit())
            if not mismo_tipo:
                continue
            o1, o2, o3 = ord(a), ord(b), ord(c)
            ascendente = (o2 - o1 == 1) and (o3 - o2 == 1)
            descendente = (o1 - o2 == 1) and (o2 - o3 == 1)
            if ascendente or descendente:
                return True
        return False

    def _cumple_reglas(self, usuario: str) -> bool:
        """Verifica que ``usuario`` cumpla todas las reglas del
        generador (longitud, tipos de caracteres exigidos, extremos,
        caracteres permitidos, repeticiones y secuencias)."""
        if len(usuario) != self.LONGITUD_USUARIO:
            return False
        if usuario[0].isdigit() or usuario[-1].isdigit():
            return False
        if usuario[0] == "_" or usuario[-1] == "_":
            return False
        if not any(c.isupper() for c in usuario):
            return False
        if not any(c.islower() for c in usuario):
            return False
        if not any(c.isdigit() for c in usuario):
            return False
        if any(c not in self._CARACTERES_PERMITIDOS for c in usuario):
            return False
        if self._tiene_repeticion_consecutiva(usuario):
            return False
        if self._tiene_secuencia(usuario):
            return False
        return True
    ...
    # === Generación ===
    # -----------------------------------------------------------------
    def generar_usuario(self) -> str:
        """Genera un nombre de usuario aleatorio de ``LONGITUD_USUARIO``
        caracteres que cumple todas las reglas comunes de formularios
        web/bancos. Usa reintentos (rápidos, dado el espacio de
        caracteres) hasta obtener un candidato válido.

        Returns:
            str: Nombre de usuario válido de 10 caracteres.
        """
        ...
        for _ in range(intentos_maximos):
            candidato = [
                ...
                random.choice(self._DIGITOS),
            ]
            restantes = self.LONGITUD_USUARIO - len(candidato)
            candidato.extend(random.choice(self._CARACTERES_PERMITIDOS) for _ in range(restantes))
            random.shuffle(candidato)
            usuario = "".join(candidato)

            if self._cumple_reglas(usuario):
                return usuario

        # Fallback (prácticamente inalcanzable): candidato fijo válido.
        return "Us3r_Segur"

    # -----------------------------------------------------------------
    # === Presentación ===
    # -----------------------------------------------------------------
    def mostrar_usuario_generado(self, usuario: str) -> None:
        """Imprime el usuario generado con el mismo estilo Cyberpunk
        usado en el resto del programa."""
        print(f"{self.colores['morado']}│ {self.colores['cyan']}👤 {self.colores['negrita']}Usuario sugerido: {self.colores['blanco']}{usuario}{self.colores['reset']}")

    def generar_usuario_interactivo(self) -> None:
        """Flujo interactivo (menú) para generar nombres de usuario de
        forma independiente, con opción de repetir la generación y
        copiar el resultado al portapapeles."""
        print(f"\n{self.colores['morado']}│ {self.colores['verde']}█{self.colores['rosa']}█{self.colores['verde']}█ {self.colores['blanco']}GENERADOR DE USUARIOS {self.colores['verde']}█{self.colores['rosa']}█{self.colores['verde']}█{self.colores['reset']}")
        print(f"{self.colores['morado']}│ {self.colores['gris']}Nombre de usuario de 10 caracteres, apto para webs y bancos{self.colores['reset']}")
        print(f"{self.colores['morado']}│{self.colores['reset']}")

        while True:
            usuario = self.generar_usuario()
            self.mostrar_usuario_generado(usuario)

            if copiar_portapapeles(usuario):
                print(f"{self.colores['verde']}│ ✓ Copiado al portapapeles{self.colores['reset']}")
            else:
                print(
                    f"{self.colores['gris']}│ ⚠ No se pudo copiar al portapapeles "
                    f"(instala 'pyperclip' o 'xclip'/'wl-clipboard'){self.colores['reset']}"
                )

            otro = input(f"\n{self.colores['morado']}│ {self.colores['azul']}¿Generar otro usuario? (s/n): {self.colores['reset']}").strip().lower()
            if otro != 's':
                break
...
def crear_parser_argumentos() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-u", "--username",
        action="store_true",
        help="Generar un nombre de usuario sugerido (10 caracteres, apto para webs/bancos) y salir.",
    )
    parser.add_argument(
        "-v", "--version",
        action="version",
...
def ejecutar_modo_no_interactivo(args: argparse.Namespace) -> int:
    ...
    if args.generate is not None:
        ...
        generador_usuarios = GeneradorUsuarios(generador.colores)
        generador_usuarios.mostrar_usuario_generado(generador_usuarios.generar_usuario())
    ...
    if args.username:
        ...
        generador_usuarios.mostrar_usuario_generado(usuario)
        return 0
...
{generador.colores['reset']}""")
    ...
    while True:
        ...
        print(f"{generador.colores['morado']}│ {generador.colores['cyan']}[9]{generador.colores['blanco']}  👤 Generar nombre de usuario")
        ...
        if opcion == "1":
            try:
                ...
                if 8 <= longitud <= 20:
                    ...
                    usuario_sugerido = generador_usuarios.generar_usuario()
                    generador_usuarios.mostrar_usuario_generado(usuario_sugerido)
                    ...
                    if guardar == 's':
                        ...
                        if nombre:
                            ...
                            generador.guardar_archivo(contrasena, nombre, subcarpeta=subcarpeta if subcarpeta else None, usuario=usuario_sugerido)
                else:
                    print(f"{generador.colores['rojo']}│ ✗ La longitud debe estar entre 8 y 20{generador.colores['reset']}")
            except ValueError:
            ...
            generador_usuarios.generar_usuario_interactivo()
